...
#
# The policy itself is generated by build_files/policy-lib.sh, which
# scripts/render-ignition.sh also uses. The installer places this same policy on
# the machine DURING installation, so the machine's first attachment to this
# image is verified like every later update (SPEC.md §spec:installer-media). Two
# copies of these rules would drift, and the drift would be silent.
# shellcheck source=/dev/null
. /ctx/image.env
# shellcheck source=/dev/null
. /ctx/policy-lib.sh
...
install -Dpm 0644 /ctx/cosign.pub "${KANTAINER_SIGNING_KEY_PATH}"

jq --arg ref "${IMAGE_REF}" --argjson scope "$(kantainer_policy_scope)" '
      .transports.docker[$ref] = $scope
...
kantainer_registries_d_yaml "${IMAGE_REF}" > /etc/containers/registries.d/kantainer.yaml
chmod 0644 /etc/containers/registries.d/kantainer.yaml

# The machine's own name for the image it is running.
#
# kantainer-attach.service - placed on the machine by the installer, not by this
# build - runs until this file exists, which is exactly "until this machine is
# running the kantainer image". It cannot use a stamp of its own: the rebase
# merges /etc into the new deployment as part of its own transaction, so
# anything written afterwards stays behind on Fedora CoreOS and the machine
# would attach, reboot, and attach all over again.
printf '%s\n' "${IMAGE_REF}" > /usr/lib/kantainer/os-image
chmod 0644 /usr/lib/kantainer/os-image
